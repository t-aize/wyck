...
fn mark_color(cx: &Ctx<'_>, profile: &Profile, mark: u16, count: usize, widest: usize) -> Hsla {
    let p = cx.f.palette;
    match cx.f.settings.tpo.color {
        TpoColor::Single => with_alpha(p.tpo, 0.95),
        TpoColor::ByPeriod => {
            let hue = (f32::from(mark) * 47.0 % 360.0) / 360.0;
            color_hsla(hue, 0.55, 0.62, 0.95)
        }
        TpoColor::ByTime => {
...
fn draw_profile(cx: &Ctx<'_>, profile: &Profile, left: f32, look: &Look, out: &mut Vec<Cmd>) {
    ...
    let line = |y: f32, color: Hsla, width: f32, dash: Option<[f32; 2]>| Cmd::Stroke {
