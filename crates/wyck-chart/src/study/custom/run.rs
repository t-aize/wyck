...
use super::super::{
    InputKind, PlotKind, SOURCES, ScriptDrawing, StudyInput, StudyOutput, ValueFormat,
};
use super::api;
...
/// Maximum number of drawings generated by one indicator instance.
pub const MAX_SCRIPT_DRAWINGS: usize = 500;
...
pub struct Columns {
    ...
    pub volume: Series,
    pub time: Series,
    /// Exact timestamps for points created from bar indices.
    pub raw_time: Arc<Vec<i64>>,
    pub hl2: Series,
    pub hlc3: Series,
    pub ohlc4: Series,
...
impl Columns {
    pub fn from_input(input: &StudyInput) -> Self {
        ...
        Self {
            ...
            raw_time: Arc::new(input.time.clone()),
...
pub struct Run {
    ...
    pub drawings: Vec<ScriptDrawing>,
    pub levels: Vec<f64>,
...
impl Script {
    ...
    pub fn compile_with(source: &str, limits: Limits) -> Result<Self, Vec<Problem>> {
        ...
        let has_draw_call = source.match_indices("draw").any(|(at, _)| {
            source[at + 4..].trim_start().starts_with('(')
                && source[..at]
                    .chars()
                    .next_back()
                    .is_none_or(|c| !c.is_ascii_alphanumeric() && c != '_')
        });
        if run.declaration.plots.is_empty() && !has_draw_call {
            warnings.push(Problem::warning(
                0,
                0,
                "the script draws nothing: add a plot(...) or draw(...) call",
            ));
        }
        if !run.declared {
            warnings.push(Problem::warning(
...
fn output_of(run: &Run) -> StudyOutput {
    ...
    output.drawings = run.drawings.clone();
    output
}
