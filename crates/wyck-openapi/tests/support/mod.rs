...
use wyck_openapi::transport::wire::{Envelope, payload};
...
/// A [`ConnectionConfig`] pointed at `server`, with a short heartbeat and request timeout so a
/// test that hangs fails fast rather than waiting on the production defaults (30s, 5s).
pub fn config(server: &MockServer) -> ConnectionConfig {
...
/// Connects a [`Client`] to `server` with [`config`]. Callers that need a non-default setting
/// (a shorter timeout to force one, for example) call [`config`] themselves instead.
