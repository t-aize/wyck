...
async fn an_unsolicited_execution_event_arrives_on_the_event_stream() {
    ...
    let event = tokio::time::timeout(Duration::from_secs(2), events.recv())
        ...
        .unwrap();
    match event {
        wyck_openapi::Event::Execution(e) => {
...
async fn an_order_error_event_and_a_trailing_stop_change_arrive_as_events() {
    ...
    assert!(matches!(
        event,
        wyck_openapi::Event::OrderError(e) if e.error_code == "POSITION_NOT_FOUND" && e.position_id == Some(5)
    ));
    ...
    assert!(matches!(
        event,
        wyck_openapi::Event::TrailingSlChanged(e) if e.stop_price == 1.09
    ));
}
