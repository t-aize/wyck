...
pub enum LineId {
    /// A working order, at its limit or stop price.
    Order(i64),
    /// The stop loss of a working order.
    OrderStopLoss(i64),
    /// The take profit of a working order.
    OrderTakeProfit(i64),
    /// An open position, at its entry price.
    Position(i64),
    /// The stop loss of an open position.
    StopLoss(i64),
    /// The take profit of an open position.
    TakeProfit(i64),
    /// A price alert.
    Alert(u64),
    /// A price being chosen for a new order, before it is sent.
    Pending(u8),
}
