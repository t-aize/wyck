...
fn hsla_rgb(color: u32, alpha: f32) -> gpui::Hsla {
    with_alpha(rgb(color), alpha)
}

/// The border around a half cell that won an imbalance.
fn outline((x0, x1): (f32, f32), y: f32, h: f32, color: gpui::Rgba) -> Cmd {
    Cmd::Rect {
        x: x0,
        y,
        w: x1 - x0,
...
fn summary(
    ...
    for (k, analysis) in analyses.iter().enumerate() {
        ...
        let delta = a.delta();
        let mut push = |text: String, y: f32, color: gpui::Rgba, bold: bool| {
