...
//! the non-idempotency notes of [`wyck::openapi::trading`]), so after a failed call the account is
...
use wyck::openapi::trading::{AmendOrderReq, AmendPositionSlTpReq, NewOrderReq};
use wyck::openapi::{Event, OpenApiError, Result as ApiResult};

use super::book::{AccountBook, Notice, Tone, explain, is_buy};
use super::math::{Contract, Summary};
...
impl Account {
    ...
    fn changed(&mut self, cx: &mut Context<Self>) {
        ...
        self.hub
            .set(ACCOUNT_OWNER, Some(Wish::spots(self.book.symbols())));
    ...
    pub fn on_event(&mut self, event: &Event, cx: &mut Context<Self>) {
        match event {
            Event::Spot(spot) => {
                ...
                if self.book.symbols().contains(&spot.symbol_id) {
                    cx.notify();
                }
            }
    ...
    where
        F: FnOnce(wyck::openapi::trading::TradingClient) -> Fut + Send + 'static,
        Fut: std::future::Future<Output = ApiResult<wyck::openapi::trading::ExecutionEvent>>
            + Send
            + 'static,
    {
