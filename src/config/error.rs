...
/// secret carries the [`crate::config::SecretKey`] it was operating on: never the secret value
...
pub enum ConfigError {
    ...
    /// [`AppConfig`](crate::config::AppConfig) could not be encoded to TOML.
    #[error("failed to serialize config to TOML: {0}")]
    Serialize(#[from] toml::ser::Error),

    /// A [`crate::config::secret::SecretStore`] backend failed. `message` is the backend's own
    ...
    /// [`crate::config::secret::SecretStore::retrieve`] rather than this variant in most call
    ...
    /// [`crate::config::secret::EncryptedFileSecretStore`]. Does not happen for well-formed
    ...
    /// [`crate::config::secret::EncryptedFileSecretStore`]. On decrypt, this most commonly
    ...
    /// [`crate::config::WyckConfig::set_active_profile`] (or similar) was given a
    /// [`crate::config::ProfileId`] that isn't in [`crate::config::AppConfig::profiles`].
