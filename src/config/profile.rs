...
//! lives here; see [`crate::config::secret`].

use serde::{Deserialize, Serialize};

/// A stable, opaque identifier for one [`ProfileConfig`], generated once when the
/// profile is created and never reused. Also doubles as the [`crate::config::secret::SecretKey`]
/// derivation input (see [`crate::config::secret::SecretKey::for_profile`]), so a profile and
...
/// module: [`crate::config`] has no knowledge of cTrader, or of any other specific
/// broker/API, on purpose. A caller using [`crate::openapi`] might use
