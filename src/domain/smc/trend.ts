 ...
 * référence, pas une simplification. `pending` expose les niveaux encore surveillés à la toute fin
 * de l'historique (résistance/support "prochains BOS/CHoCH") — même règle de classification que les
 * événements déjà réalisés, appliquée au niveau qui n'a pas encore cassé.
 */
export function detectStructureEvents(
  ...
  for (let i = 0; i < bars.length; i++) {
    ...
    if (highs[i] !== undefined) lastHigh = { index: i, price: highs[i]! };
    if (lows[i] !== undefined) lastLow = { index: i, price: lows[i]! };
  ...
  return {
    ...
    pending: {
      resistance: lastHigh
        ? { level: lastHigh.price, kind: trend === -1 ? "CHoCH" : "BOS" }
        : undefined,
      support: lastLow ? { level: lastLow.price, kind: trend === 1 ? "CHoCH" : "BOS" } : undefined,
    },
  };
}
