 ...
 * Partagée par `recordAtrTrade`/`removeAtrTrades` ci-dessous — contrairement à `readAtrTrades`,
 * garder une fonction séparée ici évite de dupliquer le `.pipe(Effect.ignore)` deux fois. */
...
export function recordAtrTrade(
  ...
  return Effect.gen(function* () {
    const trades = yield* readAtrTrades();
